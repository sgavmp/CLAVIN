...
//    	GeoParser parser = GeoParserFactory.getDefault("IndexDirectory", 50, 15 , false);
        ...
        Document doc = Jsoup.connect("http://www.fwi.co.uk/poultry/high-path-h5n8-bird-flu-in-germany-for-first-time.htm").get();
//        Document doc = Jsoup.connect("http://news.yahoo.com/german-cull-ordered-h5n8-bird-flu-confirmed-204501608.html").get();
...
//        String texto = doc.select("div.book div.body").text();
        ...
        List<ResolvedLocation> resolvedLocations = parser.parse(texto);
        
//        LocationOccurrence test = new LocationOccurrence("el puerto de ", 0);
//        List<LocationOccurrence> listaTest = new ArrayList<>();
//        listaTest.add(test);
//        List<ResolvedLocation> resolvedLocations = parser.getResolver().resolveLocations(listaTest, 50, 15, true, texto.toLowerCase());
